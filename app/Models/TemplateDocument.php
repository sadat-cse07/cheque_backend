...
//        $html .= '<th style="border:1px solid #000; padding:5px; text-align:center;">S/L</th>';
...
//        $html .= '<th style="border:1px solid #000; padding:5px; text-align:right;">Amount</th>';
...
//            $html .= '<td style="border:1px solid #000; padding:5px; text-align:right;">' . number_format($amount, 0) . '</td>';
//            $html .= '<td style="border:1px solid #000; padding:5px; text-align:center;">Cash</td>';
//            $html .= '</tr>';
//        }
//
//        // ✅ Total row inside table (aligned under Amount column)
//        $html .= '<tr style="font-weight:bold; background:#f9f9f9;">';
//        $html .= '<td colspan="4" style="border:1px solid #000; padding:8px; text-align:right; font-size:14px;">Total</td>';
//        $html .= '<td style="border:1px solid #000; padding:8px; text-align:right; font-size:14px;">' . number_format($totalAmount, 0) . '/-</td>';
//        $html .= '<td style="border:1px solid #000; padding:5px;"></td>';
//        $html .= '</tr>';
//
//        $html .= '</tbody></table>';
//
//        // ✅ In Words below the table (only once, aligned with Amount column)
//        $html .= '<div style="margin-top:10px; text-align:right; padding-right:10px;">';
//        $html .= '<p style="margin:0;"><strong>In Word: ' . $this->amountToWords($totalAmount) . '</strong></p>';
//        $html .= '</div>';
//
//        return $html;
//    }
...
class TemplateDocument extends Model
{
    ...
    /**
     * Generate cheque table HTML - NO total or inwords here
     */
    private function generateChequeTable($cheques): string
    {
        if ($cheques->isEmpty()) return '';
        ...
        $html = '<table style="width:100%; border-collapse:collapse; margin:15px 0;">';
        $html .= '<thead><tr style="background:#f0f0f0;">';
        $html .= '<th style="border:1px solid #000; padding:8px; text-align:center;">S/L</th>';
        $html .= '<th style="border:1px solid #000; padding:8px; text-align:center;">Name</th>';
        $html .= '<th style="border:1px solid #000; padding:8px; text-align:center;">Cheque No</th>';
        $html .= '<th style="border:1px solid #000; padding:8px; text-align:center;">Date</th>';
        $html .= '<th style="border:1px solid #000; padding:8px; text-align:center;">Amount</th>';
        $html .= '<th style="border:1px solid #000; padding:8px; text-align:center;">Remark</th>';
        $html .= '</tr></thead><tbody>';
        ...
        // ✅ In Words - LEFT aligned
        $html .= '<p style="margin-top:10px; text-align:left;"><strong>In Word: ' . $this->amountToWords($totalAmount) . '</strong></p>';
    ...
    /**
     * Render final document content - NO extra totals
     */
    public function renderContent(): string
    {
        $cheques = \App\Models\Cheque::whereIn('id', $this->cheque_ids ?? [])
            ->with(['vendor', 'bank'])->get();
        $vouchers = \App\Models\Voucher::whereIn('id', $this->voucher_ids ?? [])
            ->with('vendor')->get();
        ...
        // ✅ These are ONLY for {{PLACEHOLDER}} replacement in the template body
        // NOT added as extra content
        $data = [
            'DOCUMENT_DATE' => $this->document_date->format('d-m-Y'),
            'REFERENCE_NO' => $this->reference_no,
            'TITLE' => $this->title ?? $this->template->title ?? '',
            ...
            'BANK_NAME' => $cheques->first()?->bank->name ?? '',
            'SUBJECT' => $this->custom_fields['subject'] ?? '',
            'CHEQUE_TABLE' => $chequeTable,     // This already includes total + inwords
            'VOUCHER_TABLE' => $voucherTable,
            'TOTAL_AMOUNT' => number_format($totalAmount, 0),
            'TOTAL_AMOUNT_WORDS' => $this->amountToWords($totalAmount),
            'TOTAL_COUNT' => $totalCount,
    ...
    private function generateVoucherTable($vouchers): string
    {
        ...
        // Total row
        $html .= '<tr style="font-weight:bold; background:#f9f9f9;">';
        $html .= '<td colspan="3" style="border:1px solid #000; padding:8px; text-align:right;">Total</td>';
        $html .= '<td style="border:1px solid #000; padding:8px; text-align:right;">' . number_format($totalAmount, 0) . '/-</td>';
        $html .= '<td style="border:1px solid #000; padding:5px;"></td>';
        $html .= '</tr>';
